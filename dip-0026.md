...
DIP-0003 defines the provider transaction `version` field. Dash Core's v24
network upgrade introduces provider transaction payload version 3 (`ExtAddr`),
which carries the extended masternode and Platform addressing representation
together with this DIP's unified owner payout list:

| Name | Value | Description |
| --- | --- | --- |
| `ExtAddr` | 3 | Extended addressing and unified owner payout list. |

This DIP was originally specified as a separate payload version 4
(`MultiPayout`) layered above the extended-addressing version 3. Both versions
are introduced by the same deployment, so they were folded into the single
version 3 payload; no version 4 exists.

For the transactions below, a version 3 payload replaces the single
`scriptPayoutSize` and `scriptPayout` fields with a unified `payouts` list, in
the same position. The unified field is used for all owner payout
configurations, including the single-owner case. The BLS serialization scheme
is unchanged from DIP-0003. The extended addressing representation carried by
the same version is specified by Dash Core's v24 implementation and is out of
scope for this DIP.

This DIP's payout change applies to the following special transactions:

| Transaction | Special transaction type | Version 3 payout change |
| --- | --- | --- |
| ProRegTx | 1 | Replaces `scriptPayout` with `payouts`. |
| ProUpRegTx | 3 | Replaces `scriptPayout` with `payouts`. |

Other provider transactions also gain a version 3 for the extended addressing
representation; their payout-related fields are unchanged by this DIP. In
particular, this DIP does not change the existing ProUpServTx operator payout
field, which remains a separate operator-controlled service update field; the
payout list applies only to the owner-controlled portion of the masternode
reward.
...
For version 3 ProRegTx payloads, the `scriptPayoutSize` and `scriptPayout` fields
...
For version 3 ProUpRegTx payloads, the `scriptPayoutSize` and `scriptPayout`
...
Deterministic masternode state must store the owner payout list for version 3
masternodes. The previous single `scriptPayout` state field is replaced by the
unified `payouts` field for version 3 state.

State diffs must report payout-list changes as a single logical field. A
version 3 state diff that changes any payout entry, payout order, payout script,
or reward share must include the full replacement payout list.

For compatibility with pre-version-3 masternodes, implementations may internally
...
The following rules apply only to the owner payout list introduced by version 3
ProRegTx and ProUpRegTx payloads. They do not apply to the existing ProUpServTx
operator payout field. A version 3 payout list is invalid if any of the following
conditions are true:
...
version 3 payout list. A filter that matches any owner payout script in the list
...
(`DEPLOYMENT_V24`). Version 3 provider transaction payloads are only valid
after v24 activates.
...
* New version 3 registrations may use 1 to 8 owner payout entries.
* Version 3 registrar updates may change the payout list, including migrating an
  existing legacy masternode from a single owner payout script to a version 3
  payout list.
...
Implementations should treat legacy single-payout masternodes and version 3
...
1. A version 3 ProRegTx with one payout entry and `reward = 10000` is valid.
2. A version 3 ProRegTx with 8 distinct payout entries whose rewards sum to
    10000 is valid.
3. A version 3 ProRegTx with 0 payout entries is invalid.
4. A version 3 ProRegTx with 9 payout entries is invalid.
