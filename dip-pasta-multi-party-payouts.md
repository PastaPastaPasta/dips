...
1. Subtract `operatorAmount` from `masternodeReward`. The result is the owner
    reward.
2. For each owner payout entry except the last, calculate:

```text
entryAmount = floor(ownerReward * reward / 10000)
```

1. The last owner payout entry receives:

```text
ownerReward - sum(previous owner payout entry amounts)
```

1. Create one coinbase output for each owner payout entry with a non-zero
    calculated amount.
2. If `operatorAmount` is non-zero, create the operator payout output using
