...
2. `<payoutStr>`: The Dash address corresponding to the scriptPayout field of the ProRegTx. DIP-0026 replaces this with `<payoutListStr>` for version 4 ProRegTx payloads.
...
| scriptPayoutSize | compactSize uint | 1-9 | Size of the Payee Script. Replaced by `payoutsCount` in version 4 payloads defined by DIP-0026. |
| scriptPayout | Script | Variable | Payee script (p2pkh/p2sh). Replaced by `payouts` in version 4 payloads defined by DIP-0026. |
...
| 4       | Owner payout script is replaced by a payout list as defined in DIP-0026 |
...
| Registration | ProUpRegTx | Owner Only | Mode / Payout Script or Payout List / Voting Key / Operator Key |
...
For version 4 ProRegTx and ProUpRegTx payloads defined by DIP-0026, the single `scriptPayout` validation rules below are replaced by DIP-0026's payout-list validation rules.
