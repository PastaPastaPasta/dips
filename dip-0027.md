...
As a way to avoid a catastrophic failure if Platform is compromised, Core limits credit pool withdrawals. The withdrawal limits may be re-evaluated and updated periodically as the system matures.

From activation of the `v24` hard fork the limit is relative to the credit pool and net of what enters it. Let _P_ be the credit pool balance after the previous block and _D_ the balance after the block 576 blocks before it (0 if that block predates the credit pool). The allowed net drop over the window is

* _A_ = 20% of _D_, computed in duffs with integer division (rounded down), but never less than 2000 DASH.

The Asset Unlock transactions of a block must not, in total, leave the credit pool below _D_ − _A_, and must not request more DASH than the credit pool contains. Equivalently, a block may unlock at most _A_ − (_D_ − _P_), bounded to [0, _P_]. Because the rule compares balances, DASH locked into the pool inside the window (Asset Lock transactions and the Platform share of block rewards) raises _P_ and is withdrawable again at once without consuming any of the allowed drop; a deposit followed by its withdrawal leaves the limit for everyone else untouched. Nodes expose the values through the `getcreditpoolinfo` RPC.

Prior to activation of the `v24` hard fork, more restrictive limits were followed. The withdrawal would not be mined if:
...
The creditPoolBalance field represents the total amount of Dash (in duffs) locked at the block height. The value of this field must be equal to the value of this field in the previous block PLUS the sum of all Dash locked by Asset Lock transactions included in the block MINUS the sum of all Dash unlocked by Asset Unlock transactions in the block PLUS, once the masternode reward reallocation of DIP-0028 is active, the Platform share of the block's masternode reward. So, for height x, the creditPoolBalance is:

    creditPoolBalance = creditPoolBalance(x-1) + AssetLockTxs(x) - AssetUnlockTxs(x) + PlatformReward(x)
