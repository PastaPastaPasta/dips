...
Key type is `0'` (ECDSA secp256k1). The next two levels are 256-bit children, which DIP14 defines
only for secp256k1 derivation, so `1'` (BLS) is reserved on this sub-feature and must not be
used.
...
* `request id'` is a 256-bit value unique to one application login request. It is recommended to
  ...
  ephemeral secp256k1 key the application presented when requesting access, in its 33-byte
  compressed encoding. Every login then derives a distinct key, while a repeated request derives
  the same key and can be recognised as a repeat.

Wherever this document interprets 32 bytes as a 256-bit unsigned integer, the first byte is the
most significant, so the bytes are the integer's `ser256` form from DIP14.
...
Key type is `0'` (ECDSA secp256k1), as above; `1'` is reserved. The keys are used for secp256k1 ECDH.
