...
Peer to peer electronic cash systems rely on the fact they can be used with little to no trust in a third party.
We have also seen that scaling issues have dramatic effects on the usability of these systems.
...
An exponential regression estimates that the network will be at capacity around September of 2018.
However, there are outside factors that could accelerate or delay this estimate.
...
This attack is even more dramatic as the block size increases.
...
Currently, transactions in the mempool have already been verified to be standard and thus satisfy the size limit.
